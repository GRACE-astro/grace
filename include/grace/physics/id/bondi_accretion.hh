...
struct bondi_id_t {
    ...
    ) : eos(_eos), _pcoords(pcoords)
    {
        ...
        if ( par.magnetized ) {
            double Pc = par.Tc * std::pow(par.Tc/par.K,par.n) ; // pressure at rc 
            ...
            // find u^t 
            double utc = std::sqrt((grr * SQR( par.uc)+1)/(-gtt) ) ; 
            // get Lorentz factor 
            double Wc = std::sqrt(1-2*par.M/par.rc) * utc ; 
            // find B^r (r=r_c)
            double Brc = Wc * std::sqrt( smallb2_c / (grr * (1+grr*SQR(par.uc)))) ; 
