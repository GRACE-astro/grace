...
enum bc_t: uint8_t {BC_OUTFLOW=0, BC_LAGRANGE_EXTRAP} ; 
...
struct face_phys_bc_k {
    ...
    ) 
    {
        ...
        int8_t const dir[] = {
            (face_id == 0 ) ? -1 : (face_id==1 ? +1 : 0) ,
            (face_id == 2 ) ? -1 : (face_id==3 ? +1 : 0) ,
            (face_id == 4 ) ? -1 : (face_id==5 ? +1 : 0)
        } ; 
        ...
        auto src_sv = Kokkos::subview(
            src, 
            VEC(Kookkos::ALL(), Kokkos::ALL(), Kokkos::ALL()),
            iv, quad_id 
        ) ; 
        ...
        for (int ig = lmin; ig != lmax; ig += idir) {
            ...
            auto const I = (faceb2 == 0) ? ig : i;
            auto const J = (faceb2 == 1) ? ig : (faceb2 == 0 ? i : j);
            auto const K = (faceb2 == 2) ? ig : j;
