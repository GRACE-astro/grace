...
) 
{
    ...
    // quad_id 
    std::array<std::array<std::vector<size_t>,3>,3> qid, qid_cbuf, qid_def  ;
    std::array<std::array<std::vector<uint8_t>,3>,3> eid, eid_cbuf, eid_def ;
    std::array<std::array<std::vector<std::array<int8_t,3>>,3>,3> dir, dir_cbuf, dir_def ; 
    std::array<std::array<std::unordered_set<task_id_t>,3>,3> dependencies, dependencies_cbuf ;
    
    auto const insert_dependency = [&] (task_id_t tid, std::unordered_set<task_id_t>& deps)
    {
    ...
    auto get_cids_edge_face = [&] (uint8_t iface, const int dir[3]) -> std::tuple<uint8_t, uint8_t> {
        uint8_t idir ; 
        uint8_t isign ; 
    ...
    auto get_cid_corner_face = [&] (const int dir[3]) -> uint8_t {
        uint8_t idir ; 
        uint8_t isign ; 
        for( uint8_t i=0; i<3; ++i) {
            if ( dir[i] ) {
    ...
    // dependencies 
    auto unpack_dependencies = [&] (int const& kind, gpu_task_desc_t const& d, bool is_cbuf) ->  bool 
    {
        if constexpr (kind == FACE) {
            // nothing to do here 
            return ; 
        } else if constexpr (kind == EDGE) {
            auto& edge = ghost_array[std::get<0>(d)].edges[std::get<1>(d)] ; 
            ...
            if ( type == FACE ) {
                ...
                auto& face = ghost_array[std::get<0>(d)].faces[face_idx] ;
            ...
            if ( type == FACE ) { // type stands for BC type 
                ...
                auto& edge = ghost_array[std::get<0>(d)].edges[edge_idx] ;
                ...
                // to be deferred 
                for( int i=0; i<3; ++i) {
                    auto& edge = ghost_array[std::get<0>(d)][c2e[std::get<1>(d)][i]] ; 
    ...
    for( int kind=0; kind<3 ; ++kind) { // element kind 
        for( auto const& d: phys_bc_tasks[kind]) { 
            ...
            auto is_deferred = unpack_dependencies(kind, d, is_cbuf);
            if ( is_cbuf ) {
            ...
            if (is_cbuf or is_deferred) {
                qid_def[kind][type].push_back(_qid) ; 
                eid_def[kind][type].push_back(_eid) ; 
                dir_def[kind][type].emplace_back(dx,dy,dz) ; 
            }
            set_task_id(d) ; 
        }
    }
