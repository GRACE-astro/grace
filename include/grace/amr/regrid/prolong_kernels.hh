...
)
{
    ...
    task._run = [functor, policy
        ...
        , qin_host, qout_host, have_fine_x_host, have_fine_y_host, have_fine_z_host
        ...
        Kokkos::fence();
        {
            // PRE-PROLONG checks: coarse divB + conservation at copied faces
            ...
            auto U = functor.data_out.face_staggered_fields_x;
            auto V = functor.data_out.face_staggered_fields_y;
            auto W = functor.data_out.face_staggered_fields_z;
            size_t _n = prolong_nx, _g = prolong_ngz;

            // 1) Coarse parent div-B on OLD grid
            for (size_t iq = 0; iq < qin_host.size(); iq += P4EST_CHILDREN) {
                size_t qid_c = qout_host[iq / P4EST_CHILDREN];
                double max_cdivb = 0;
                size_t mi=0, mj=0, mk=0;
                for (size_t i = _g; i < _g+_n; ++i)
                    for (size_t j = _g; j < _g+_n; ++j)
                        for (size_t k = _g; k < _g+_n; ++k) {
                            double d = (U(i+1,j,k,0,qid_c) - U(i,j,k,0,qid_c))
                                     + (V(i,j+1,k,0,qid_c) - V(i,j,k,0,qid_c))
                                     + (W(i,j,k+1,0,qid_c) - W(i,j,k,0,qid_c));
                            double ad = d < 0 ? -d : d;
                            if (ad > max_cdivb) { max_cdivb = ad; mi=i; mj=j; mk=k; }
                        }
                if (max_cdivb > 1e-14)
                    GRACE_TRACE("[PRE-PROLONG] coarse qid={} max|divB|={:.6e} at ({},{},{})",
                        qid_c, max_cdivb, mi, mj, mk);
                else
                    GRACE_TRACE("[PRE-PROLONG] coarse qid={} divB clean ({:.6e})", qid_c, max_cdivb);
            }

            // 2) Conservation check: 4*U_coarse vs sum of copied fine face values
            for (size_t iq = 0; iq < qin_host.size(); ++iq) {
                size_t qid = qin_host[iq];
                int8_t ichild = iq % P4EST_CHILDREN;
                int ox = ((ichild>>0)&1) * (int)(_n/2);
                int oy = ((ichild>>1)&1) * (int)(_n/2);
                int oz = ((ichild>>2)&1) * (int)(_n/2);
                size_t qid_c = qout_host[iq / P4EST_CHILDREN];
                ...
                for (int side = 0; side < 2; ++side) {
                    if (!hfx[side]) continue;
                    size_t fi = side ? _n + _g : _g;
                    size_t ci = _g + ox + side * (int)(_n/2);
                    double max_diff = 0;
                    size_t worst_cj=0, worst_ck=0;
                    for (size_t cj = _g + oy; cj < _g + oy + (int)(_n/2); ++cj) {
                        for (size_t ck = _g + oz; ck < _g + oz + (int)(_n/2); ++ck) {
                            size_t fj0 = _g + 2*(cj - _g - oy);
                            size_t fk0 = _g + 2*(ck - _g - oz);
                            double sum_f = u(fi,fj0,fk0,0,qid) + u(fi,fj0+1,fk0,0,qid)
                                         + u(fi,fj0,fk0+1,0,qid) + u(fi,fj0+1,fk0+1,0,qid);
                            double diff = sum_f - 4.0 * U(ci,cj,ck,0,qid_c);
                            double ad = diff < 0 ? -diff : diff;
                            if (ad > max_diff) { max_diff = ad; worst_cj = cj; worst_ck = ck; }
                        }
                    }
                    if (max_diff > 1e-14)
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} Bx side={} max|4U-Σu|={:.6e} at coarse ({},{},{})",
                            iq, ichild, side, max_diff, ci, worst_cj, worst_ck);
                    else
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} Bx side={} OK ({:.6e})", iq, ichild, side, max_diff);
                }
                // By faces
                for (int side = 0; side < 2; ++side) {
                    if (!hfy[side]) continue;
                    size_t fj = side ? _n + _g : _g;
                    size_t cj = _g + oy + side * (int)(_n/2);
                    double max_diff = 0;
                    size_t worst_ci=0, worst_ck=0;
                    for (size_t ci = _g + ox; ci < _g + ox + (int)(_n/2); ++ci) {
                        for (size_t ck = _g + oz; ck < _g + oz + (int)(_n/2); ++ck) {
                            size_t fi0 = _g + 2*(ci - _g - ox);
                            size_t fk0 = _g + 2*(ck - _g - oz);
                            double sum_f = v(fi0,fj,fk0,0,qid) + v(fi0+1,fj,fk0,0,qid)
                                         + v(fi0,fj,fk0+1,0,qid) + v(fi0+1,fj,fk0+1,0,qid);
                            double diff = sum_f - 4.0 * V(ci,cj,ck,0,qid_c);
                            double ad = diff < 0 ? -diff : diff;
                            if (ad > max_diff) { max_diff = ad; worst_ci = ci; worst_ck = ck; }
                        }
                    }
                    if (max_diff > 1e-14)
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} By side={} max|4V-Σv|={:.6e} at coarse ({},{},{})",
                            iq, ichild, side, max_diff, worst_ci, cj, worst_ck);
                    else
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} By side={} OK ({:.6e})", iq, ichild, side, max_diff);
                }
                // Bz faces
                for (int side = 0; side < 2; ++side) {
                    if (!hfz[side]) continue;
                    size_t fk = side ? _n + _g : _g;
                    size_t ck = _g + oz + side * (int)(_n/2);
                    double max_diff = 0;
                    size_t worst_ci=0, worst_cj=0;
                    for (size_t ci = _g + ox; ci < _g + ox + (int)(_n/2); ++ci) {
                        for (size_t cj = _g + oy; cj < _g + oy + (int)(_n/2); ++cj) {
                            size_t fi0 = _g + 2*(ci - _g - ox);
                            size_t fj0 = _g + 2*(cj - _g - oy);
                            double sum_f = w(fi0,fj0,fk,0,qid) + w(fi0+1,fj0,fk,0,qid)
                                         + w(fi0,fj0+1,fk,0,qid) + w(fi0+1,fj0+1,fk,0,qid);
                            double diff = sum_f - 4.0 * W(ci,cj,ck,0,qid_c);
                            double ad = diff < 0 ? -diff : diff;
                            if (ad > max_diff) { max_diff = ad; worst_ci = ci; worst_cj = cj; }
                        }
                    }
                    if (max_diff > 1e-14)
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} Bz side={} max|4W-Σw|={:.6e} at coarse ({},{},{})",
                            iq, ichild, side, max_diff, worst_ci, worst_cj, ck);
                    else
                        GRACE_TRACE("[PRE-PROLONG:CONSERV] iq={} child={} Bz side={} OK ({:.6e})", iq, ichild, side, max_diff);
                }
            }
        }
        #endif
        ...
        Kokkos::fence();
        {
            ...
            for (size_t iq = 0; iq < qin_host.size(); ++iq) {
                ...
                // print all boundary face values (var 0) for faces that have fine data
                if (hfx[0]) {
                    GRACE_TRACE("  Bx lower-x face (i={}):", _g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    u({},{},{},0,{}) = {:.15e}", _g, _g+tj, _g+tk, qid, u(_g, _g+tj, _g+tk, 0, qid));
                }
                if (hfx[1]) {
                    GRACE_TRACE("  Bx upper-x face (i={}):", _n+_g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    u({},{},{},0,{}) = {:.15e}", _n+_g, _g+tj, _g+tk, qid, u(_n+_g, _g+tj, _g+tk, 0, qid));
                }
                if (hfy[0]) {
                    GRACE_TRACE("  By lower-y face (j={}):", _g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    v({},{},{},0,{}) = {:.15e}", _g+tj, _g, _g+tk, qid, v(_g+tj, _g, _g+tk, 0, qid));
                }
                if (hfy[1]) {
                    GRACE_TRACE("  By upper-y face (j={}):", _n+_g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    v({},{},{},0,{}) = {:.15e}", _g+tj, _n+_g, _g+tk, qid, v(_g+tj, _n+_g, _g+tk, 0, qid));
                }
                if (hfz[0]) {
                    GRACE_TRACE("  Bz lower-z face (k={}):", _g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    w({},{},{},0,{}) = {:.15e}", _g+tj, _g+tk, _g, qid, w(_g+tj, _g+tk, _g, 0, qid));
                }
                if (hfz[1]) {
                    GRACE_TRACE("  Bz upper-z face (k={}):", _n+_g);
                    for (size_t tj = 0; tj <= _n; ++tj)
                        for (size_t tk = 0; tk <= _n; ++tk)
                            GRACE_TRACE("    w({},{},{},0,{}) = {:.15e}", _g+tj, _g+tk, _n+_g, qid, w(_g+tj, _g+tk, _n+_g, 0, qid));
                }
