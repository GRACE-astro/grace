                          ...
                          , hsize_t local_quad_offset) // had isaux 
{
    ...
    //octree_slicer.find_sliced_cells_for_sphere(20,0.05); Does not work. What bullshit
    ...
    for( auto const& vname: varlist )
    {
        ...
        HDF5_CALL(mem_space_id, H5Screate_simple(1, sliced_slab_count, NULL)) ;
        HDF5_CALL(mem_space_id_glob, H5Screate_simple(1, sliced_slab_count, NULL)) ;
        ...
        Kokkos::parallel_for("Copy sliced cells", 
            ...
            KOKKOS_LAMBDA(const int idx) {
                ...
                d_sliced_buffer(idx) = sview(x, y, z, q); // Access simulation data
            }
        ); 
        ...
        printf("Writing to dataset %s\n", sliced_dset_name.c_str());
        HDF5_CALL(err, H5Dwrite(sliced_dset_id, H5T_NATIVE_DOUBLE, mem_space_id, mem_space_id_glob, dxpl, h_sliced_buffer.data()));
        HDF5_CALL(err, H5Dclose(sliced_dset_id));
        HDF5_CALL(err, H5Sclose(mem_space_id)) ; 
        HDF5_CALL(err, H5Sclose(mem_space_id_glob)) ; 
        //HDF5_CALL(err, H5Dwrite(sliced_dset_id, H5T_NATIVE_DOUBLE, space_id, space_id_glob, dxpl, h_sliced_buffer.data()));
    }
