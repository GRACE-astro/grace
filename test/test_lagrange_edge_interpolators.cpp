...
TEST_CASE("lagrange_interp_edge", "[lagrange_interp_edge]")
{
    ...
    /****************************************************************/
    /* We by testing the second order (linear)                      */
    ...
    constexpr size_t edgeDir= 2; // z dir
    ...
    parallel_for("fill_data_edge",policy_edges<edgeDir>,
    KOKKOS_LAMBDA( int i, int j, int k) {
        auto const& xyz_edge = coords_edge<edgeDir>(i,j,k) ; 
